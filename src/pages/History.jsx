...
export default function History() {
  ...
  const handleCancel = async (id, billNo) => {
    if (!confirm(`Cancel Bill #${billNo}?`)) return
    try {
      await cancelBill(id)
  ...
  const dayTotal = bills.filter(b => b.status === 'paid').reduce((s, b) => s + Number(b.total), 0)
    ...
    <div className="page">
      ...
      <div className="stat-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: '1.25rem' }}>
        ...
        <div className="stat-card">
          ...
          <div className="stat-val">{bills.filter(b => b.status === 'paid').length}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Cancelled</div>
          <div className="stat-val">{bills.filter(b => b.status === 'cancelled').length}</div>
        </div>
      </div>
                    ...
                    {bills.map(b => (
                      <>
                        <tr key={b.id} style={{ cursor: 'pointer' }} onClick={() => setExpanded(expanded === b.id ? null : b.id)}>
                          ...
                          <td>
                            {b.status === 'paid' && (
                              <button className="btn-ghost btn btn-sm" onClick={e => { e.stopPropagation(); handleCancel(b.id, b.bill_number) }}>
                                Cancel
                              </button>
                            )}
                          </td>
