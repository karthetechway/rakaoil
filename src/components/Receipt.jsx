    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          #chekku-receipt-root * { visibility: visible !important; color: #000 !important; }
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: 8 }}>
          <div style={{ fontWeight: 'bold', fontSize: 15 }}>{SHOP.NAME}</div>
          {SHOP.ADDRESS && <div style={{ fontSize: 12 }}>{SHOP.ADDRESS}</div>}
          {SHOP.PHONE && <div style={{ fontSize: 12 }}>Ph: {SHOP.PHONE}</div>}
          {SHOP.EMAIL && <div style={{ fontSize: 12 }}>Email: {SHOP.EMAIL}</div>}
          {SHOP.GSTIN && <div style={{ fontSize: 12 }}>{SHOP.GSTIN}</div>}
        </div>
        ...
        <div style={{ textAlign: 'center', fontSize: 12 }}>
          <div>Thank you for your purchase of RAKA oil products!</div>
          <div>எங்களது ராகா எண்ணெய் பொருட்களை வாங்கியதற்கு நன்றி !</div>
        </div>
      </div>
    </>
  )
